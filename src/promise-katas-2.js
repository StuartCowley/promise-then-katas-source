...
const dog = async () => {
  ...
  const naughtyDog = response.data.dogs.reduce((acc, curr) =>
    acc.naughty > curr.naughty ? acc : curr
  );
  return naughtyDog;
};
...
const joke = async () => {
  ...
  const joke = await Promise.all([questionPromise, answerPromise]);
  return { question: joke[0].joke, answer: joke[1].answer };
};
